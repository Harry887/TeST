...
from tst.data.infinite import Infinite
...
class Exp(BaseExp):
    ...
    def x_u_split(self, labels):
        ...
        if self.expand_labels or self.num_labeled < self.batch_size_per_device:
            num_expand_x = math.ceil(self.batch_size_per_device * self.eval_step / self.num_labeled)
    ...
    def get_data_loader(self):
        ...
        self.batch_size_per_device = self.batch_size // dist.get_world_size()
        ...
        base_dataset = build_dataset(self.dataset, self.dataset_root, split="train", download=False)
        labeled_idxs, unlabeled_idxs = self.x_u_split(base_dataset.labels)
        labeled_dataset = build_dataset(
            self.dataset, self.dataset_root, labeled_idxs, split="train", return_index=True, transform=transform_labeled
        )
        ...
        test_dataset = build_dataset(
            self.dataset, self.dataset_root, split="test", transform=transform_val, return_index=False, download=False
        )

        from torch.utils.data.distributed import DistributedSampler

        # TODO Figure out difference between Infinite(DistributedSampler) and InfiniteSampler
        train_sampler_labeled = DistributedSampler(labeled_dataset, shuffle=True, seed=self.seed if self.seed else 0)
        train_sampler_unlabeled = DistributedSampler(
            unlabeled_dataset, shuffle=True, seed=self.seed if self.seed else 0
        )

        labeled_trainloader = DataLoader(
            labeled_dataset,
            sampler=Infinite(train_sampler_labeled),
        ...
        unlabeled_trainloader = DataLoader(
            unlabeled_dataset,
            sampler=Infinite(train_sampler_unlabeled),
            batch_size=self.batch_size_per_device * self.mu,
            num_workers=self.data_num_workers,
            drop_last=True,
        )
